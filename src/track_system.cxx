...
struct RdrBuildScratch
{
    ...
    TrackHighWorkVector<SRL::Math::Types::Vector3D> verts{};
    TrackHighWorkVector<SRL::Types::Polygon> faces{};
    TrackHighWorkVector<SRL::Types::Attribute> attrs{};
};
...
static constexpr size_t kWorkRamSlideSafeFloorBytes = kWorkRamHardFloorBytes;
...
static constexpr size_t kLowWorkRamSoftFloorBytes = 320u * 1024u;
...
static constexpr size_t kLodMandatoryBandSegmentCount =
    kLodBand64Count + kLodBand32Count + kLodBand16Count;
...
static void CopyFaceSlotsToScratch(const TrackLowWorkI16Vector& src,
                                   TrackHighWorkI16Vector& dst)
{
    dst.assign(src.begin(), src.end());
}

static void RestoreFaceSlotsFromScratch(TrackLowWorkI16Vector& dst,
                                        const TrackHighWorkI16Vector& src)
{
    dst.assign(src.begin(), src.end());
}
...
{
    const size_t desired = aggressive ? size : (size + slack);
...
{
    decltype(blob.bytes){}.swap(blob.bytes);
    blob.size = 0;
    blob.loaded = false;
}
...
static void TrimRuntimeBlobScratchCaches(bool aggressive)
{
    if (aggressive)
    {
        ...
        decltype(g_rdrBuildScratch.verts){}.swap(g_rdrBuildScratch.verts);
        decltype(g_sdrBuildScratch.verts){}.swap(g_sdrBuildScratch.verts);
        decltype(g_rdrBuildScratch.faces){}.swap(g_rdrBuildScratch.faces);
        decltype(g_sdrBuildScratch.faces){}.swap(g_sdrBuildScratch.faces);
        decltype(g_rdrBuildScratch.attrs){}.swap(g_rdrBuildScratch.attrs);
        decltype(g_sdrBuildScratch.attrs){}.swap(g_sdrBuildScratch.attrs);
        return;
    }
...
static void ConfigureStreamedRendererDefaults(TrackRenderer& renderer)
{
    ...
    // shared scratch vectors; compact to the current segment size immediately
    // after each rebuild so the 20-slot window stays close to constant.
...
static constexpr uint8_t kReusableTrackSlotReuseDelayFrames = 6u;
...
struct DecodedTgaTexture
{
    ...
    TrackHighWorkVector<SRL::Types::HighColor> palette{};
    TrackHighWorkVector<uint8_t> pixels{};
};
...
static bool DecodePalettedTgaMemory(const uint8_t* data, size_t size, DecodedTgaTexture& out)
{
    ...
    if (colorMapType == 1 && imageType == 1)
    {
        ...
        TrackHighWorkVector<SRL::Types::HighColor> srcPalette{};
        ...
        TrackHighWorkVector<SRL::Types::HighColor> compactPalette{};
        TrackHighWorkVector<uint8_t> compactIndices{};
...
TrackLowWorkVector<TrackSystem::SegmentHandle> TrackSystem::BuildSegmentHandleTable()
{
    segmentPool_.Reset();
    TrackLowWorkVector<SegmentHandle> handles;
...
bool TrackSystem::PreloadFullTrackFamilyLodCache()
{
    ...
    if (fullTrackFamilyCacheReady_ && kEnableTrackRuntimeStabilization)
    {
        ...
        const uint8_t preloadLodCount = 1u;
        for (auto& family : seg1FamilySlots_)
        {
            for (uint8_t preloadLodIndex = 0; preloadLodIndex < preloadLodCount; ++preloadLodIndex)
            {
    ...
    {
        SRL::Debug::Print(1, 21, "TRK fam catalog fam:%u b:%u 8:%u/%u 16:%u/%u",
                          ...
                          loadedLodCounts[1],
                          failedLodCounts[1]);
        SRL::Debug::Print(1, 22, "TRK fam lod32:%u/%u 64:%u/%u",
                          loadedLodCounts[2],
                          failedLodCounts[2],
                          loadedLodCounts[3],
                          failedLodCounts[3]);
...
{
    outSlots.clear();
    if (outSlots.capacity() < 256u) outSlots.reserve(256u);
                                          ...
                                          bool bypassUploadBudget)
{
    ...
    for (uint8_t delta = 1u; delta < 4u; ++delta)
    {
        if (preferredLodIndex >= delta && tryExistingSlot(static_cast<uint8_t>(preferredLodIndex - delta)))
        {
            return true;
        }
        const uint8_t higher = static_cast<uint8_t>(preferredLodIndex + delta);
        if (higher < 4u && tryExistingSlot(higher))
        {
            return true;
        }
    }
    ...
    if (loaded && ready_) ++textureUploadsThisFrame_;
    if (tryExistingSlot(preferredLodIndex)) return true;
    for (uint8_t lodIndex = 0u; lodIndex < 4u; ++lodIndex)
    {
        if (tryExistingSlot(lodIndex)) return true;
    }
    return false;
}
...
bool TrackSystem::RebuildSafeSegmentEntry(SegmentRenderEntry& entry)
{
    ...
    if (kEnableTrackLodBandsInStabilization)
    {
        ...
        else
        {
            ...
            {
                desiredLodIndex = entry.lodState.currentLodIndex;
            }
        }
    }
    else if (entry.lodState.currentLodIndex <= 3u)
    ...
    TrackHighWorkI16Vector previousFaceSlots{};
    CopyFaceSlotsToScratch(entry.lodState.currentFaceSlots, previousFaceSlots);
    bool rebuilt = false;
    uint8_t appliedLodIndex = desiredLodIndex;
    const bool requireExactLod =
        kEnableTrackRuntimeStabilization && kEnableTrackLodBandsInStabilization;
    const int lowestLodAttempt = requireExactLod ? static_cast<int>(desiredLodIndex) : 0;
    ...
    if (!rebuilt)
    {
        RestoreFaceSlotsFromScratch(entry.lodState.currentFaceSlots, previousFaceSlots);
        return false;
    }
...
bool TrackSystem::TryGetWindowLogicalRank(int32_t segmentId, size_t& outRank) const
{
    ...
    if (static_cast<size_t>(segmentId) >= activeWindowLogicalRankBySegmentId_.size()) return false;
    const int16_t rank = activeWindowLogicalRankBySegmentId_[static_cast<size_t>(segmentId)];
...
void TrackSystem::RebuildActiveWindowLookupTables()
{
    ...
    const size_t lookupSize = static_cast<size_t>(std::max<int32_t>(0, totalSegmentCount_)) + 1u;
    activeWindowEntryIndexBySegmentId_.assign(lookupSize, static_cast<int16_t>(-1));
    activeWindowLogicalRankBySegmentId_.assign(lookupSize, static_cast<int16_t>(-1));
    if (lookupSize == 0 || segmentRenderers_.empty() || totalSegmentCount_ == 0)
    {
    ...
    for (size_t i = 0; i < segmentRenderers_.size(); ++i)
    {
        const int32_t id = segmentRenderers_[i].id;
        if (id <= 0 || static_cast<size_t>(id) >= lookupSize) continue;
        activeWindowEntryIndexBySegmentId_[static_cast<size_t>(id)] = static_cast<int16_t>(i);
    }
    ...
    if (startId > 0)
    {
        ...
        {
            const int32_t expectedId = WrapSegmentIdToRange(
                startId + (dir > 0 ? static_cast<int32_t>(logicalRank)
                                   : -static_cast<int32_t>(logicalRank)),
                totalSegmentCount_);
            if (expectedId <= 0 || static_cast<size_t>(expectedId) >= lookupSize) continue;
            activeWindowLogicalRankBySegmentId_[static_cast<size_t>(expectedId)] =
...
{
    if (segmentId <= 0 || totalSegmentCount_ == 0) return nullptr;
    RebuildActiveWindowLookupTables();
    if (static_cast<size_t>(segmentId) >= activeWindowEntryIndexBySegmentId_.size()) return nullptr;
    const int16_t idx = activeWindowEntryIndexBySegmentId_[static_cast<size_t>(segmentId)];
    if (idx < 0 || static_cast<size_t>(idx) >= segmentRenderers_.size()) return nullptr;
    return &segmentRenderers_[static_cast<size_t>(idx)];
}
...
void TrackSystem::UpdateDesiredStabilizedWindowLodTargets()
{
    ...
    for (size_t logicalRank = 0; logicalRank < segmentRenderers_.size(); ++logicalRank)
    {
        ...
        SegmentRenderEntry* entry = FindWindowEntryByIdFast(expectedId);
        if (!entry) continue;
...
{
    if (kEnableDeterministicStabilizedSlide) return false;
...
bool TrackSystem::ApplyStabilizedLodForLogicalRank(size_t logicalRank)
{
    ...
    SegmentRenderEntry* entry = FindWindowEntryByIdFast(segmentId);
    if (!entry || !entry->renderer || !entry->lodState.ready) return false;
                                                      ...
                                                      int32_t nextStartId)
{
    ...
    slideHwrTraceSegmentId_ = static_cast<int16_t>(nextId);
    ...
    auto missingPrefetchSlots = [&]() -> uint32_t
    {
        if (slidePrefetchFamilyIds_.empty()) return 0u;
        if (slidePrefetchFaceSlots_.size() != slidePrefetchFamilyIds_.size())
        {
            return static_cast<uint32_t>(slidePrefetchFamilyIds_.size());
        }
        return CountMissingOrDeadRequiredFaceTextureSlots(slidePrefetchFaceSlots_,
                                                          &slidePrefetchFamilyIds_);
    };

    const bool hadReadyPrefetch =
        prefetchReady() &&
    ...
    else
    {
        ...
        const bool criticalWorkRam =
            (freeValid && freeBytes <= (kWorkRamHardFloorBytes + (8u * 1024u))) ||
            (lowFreeValid && lowFreeBytes <= kLowWorkRamHardFloorBytes);
        ...
        if (!prefetchReady())
        {
            ...
            SRL::Debug::Print(1, 11, "PKG pf fail id:%d rr:%d l8:%d ms:%u tx:%u p16:%u rs:%u",
                              nextId,
                              slideScratchRenderer_ ? 1 : 0,
                              slidePrefetchLod8Ready_ ? 1 : 0,
                              missingPrefetchSlots(),
    ...
    {
        slideHwrTraceFlags_ |= kSlideHwrTracePrepareFailBit;
        SRL::Debug::Print(1, 11, "PKG tail miss id:%d rr:%d l8:%d ms:%u",
                          nextId,
                          slideScratchRenderer_ ? 1 : 0,
                          slidePrefetchLod8Ready_ ? 1 : 0,
                          missingPrefetchSlots());
    ...
    struct BoundaryPrepared
    {
        ...
        TrackHighWorkI16Vector faceSlots{};
    };
    ...
    slot.id = static_cast<int16_t>(nextId);
    slot.logicalSegmentCount = 1;
    slot.center = slidePrefetchCenter_;
    slot.lodState.ready = true;
    slot.lodState.hasPerFaceRankOffsets = false;
    slot.lodState.faceFamilyIds.swap(slidePrefetchFamilyIds_);
    slot.lodState.faceRankOffsets.assign(slot.lodState.faceFamilyIds.size(), 0u);
    slot.lodState.currentFaceSlots.swap(slidePrefetchFaceSlots_);
    ...
    meta.id = static_cast<int16_t>(nextId);
    ...
    if (!foundStart)
    {
        activeWindowHead_ = dropIdx % segmentRenderers_.size();
    }
                                                   ...
                                                   int32_t nextStartId)
{
    ...
    auto prefetchReady = [&]() -> bool
    {
        ...
        if (!slidePrefetchRendererReady_) return false;
    ...
    auto missingPrefetchSlots = [&]() -> unsigned
    {
        if (slidePrefetchFaceSlots_.size() != slidePrefetchFamilyIds_.size())
        {
            return static_cast<unsigned>(slidePrefetchFamilyIds_.size());
        }
        unsigned missing = 0;
        for (size_t i = 0; i < slidePrefetchFamilyIds_.size(); ++i)
        {
            if (slidePrefetchFamilyIds_[i] == 0) continue;
            if (slidePrefetchFaceSlots_[i] >= 0) continue;
            ++missing;
        }
        return missing;
    };

    const bool hadReadyPrefetch = prefetchReady();
    if (hadReadyPrefetch)
    {
        ++runtimePrefetchHitsThisFrame_;
        slideHwrTraceAfterBuildPrefetch_ = static_cast<uint32_t>(GetHighWorkRamFreeBytesSafe());
    ...
    slideBackBuffer_.incomingSegmentId = static_cast<int16_t>(nextId);
    slideBackBuffer_.outgoingSegmentId = segmentRenderers_[dropIdx].id;
    slideBackBuffer_.nextStartId = static_cast<int16_t>(nextStartId);
    slideBackBuffer_.incomingCenter = slidePrefetchCenter_;
    const size_t windowCount = segmentRenderers_.size();
    const size_t incomingLogicalRank =
    ...
    const bool incomingLodExact =
        slidePrefetchLod8Ready_ &&
        slidePrefetchFaceSlots_.size() == slidePrefetchFamilyIds_.size() &&
        !HasMissingRequiredFaceTextureSlots(slidePrefetchFaceSlots_,
                                           &slidePrefetchFamilyIds_);
    if (!incomingLodExact)
    {
        slideHwrTraceFlags_ |= kSlideHwrTracePrepareFailBit;
    ...
    slideBackBuffer_.incomingFamilyIds.swap(slidePrefetchFamilyIds_);
    slideBackBuffer_.incomingFaceSlots.swap(slidePrefetchFaceSlots_);
...
bool TrackSystem::CommitStabilizedSlideBackBuffer()
{
    ...
    if (!foundStart)
    {
        activeWindowHead_ = slideBackBuffer_.dropIdx % segmentRenderers_.size();
    }
...
bool TrackSystem::BuildSegmentIntoPrefetch(int32_t segmentId, bool allowSlotWarmup)
{
    ...
    if (kEnableTrackRuntimeStabilization)
    {
        const bool prefetchAlreadyReady =
        ...
        if (!allowSlotWarmup)
        {
            const bool needsLightPrefetch =
                (slidePrefetchSegmentId_ != segmentId) ||
                slidePrefetchFamilyIds_.empty();
            if (needsLightPrefetch)
            {
        ...
        FamilySlotVector& prefetchFamilySlots = slidePrefetchFamilySlotsScratch_;
        prefetchFamilySlots.clear();
        if (!seg1FamilySlots_.empty())
        {
            if (prefetchFamilySlots.capacity() <
                (seg1FamilySlots_.size() + slidePrefetchFamilyIds_.size()))
            {
                prefetchFamilySlots.reserve(seg1FamilySlots_.size() + slidePrefetchFamilyIds_.size());
            }
            prefetchFamilySlots.insert(prefetchFamilySlots.end(),
                                       seg1FamilySlots_.begin(),
                                       seg1FamilySlots_.end());
        }
        else
        {
            (void)BuildTrackFamilyLodSlots(prefetchFamilySlots);
        }
        for (size_t fi = 0; fi < slidePrefetchFamilyIds_.size(); ++fi)
        {
            const uint16_t fam = slidePrefetchFamilyIds_[fi];
            if (fam == 0) continue;
            if (FindFamilySlot(prefetchFamilySlots, fam)) continue;
            Seg1FamilySlotEntry slotEntry{};
            slotEntry.familyId = fam;
            slotEntry.lodSlots = { No_Texture, No_Texture, No_Texture, No_Texture };
            prefetchFamilySlots.push_back(slotEntry);
        }
        ...
        // Normal prefetch should not keep expanding residency aggressively while
        // the car is still inside the current segment. Reserve bypass for the
        // critical slide path only.
        ...
        EnsureVectorCapacityFloor(slidePrefetchFamilyIds_, slotFaceCapacityFloor_);
        EnsureVectorCapacityFloor(slidePrefetchFaceSlots_, slotFaceCapacityFloor_);
        slidePrefetchRendererReady_ = static_cast<bool>(slideScratchRenderer_);
        slidePrefetchLod8Ready_ =
            rebuilt &&
            !HasMissingRequiredFaceTextureSlots(slidePrefetchFaceSlots_, &slidePrefetchFamilyIds_);
        familyWorkingSetDirty_ = true;
        return slideScratchRenderer_ &&
               slidePrefetchSegmentId_ == segmentId &&
               !slidePrefetchFamilyIds_.empty();
    }
...
void TrackSystem::PrimeRuntimeScratchCapacities()
{
    ...
    // In stabilized runtime we want headroom more than prime-at-max throughput.
    // The renderer-build scratch can reserve on demand later; keeping max-size
    // HWR buffers resident for the whole race leaves too little room for slides.
    if (!kEnableTrackRuntimeStabilization)
    {
    ...
    for (auto& entry : segmentRenderers_)
    {
        EnsureVectorCapacityFloor(entry.lodState.faceFamilyIds, maxFaces);
        EnsureVectorCapacityFloor(entry.lodState.faceRankOffsets, maxFaces);
        EnsureVectorCapacityFloor(entry.lodState.currentFaceSlots, maxFaces);
        if (entry.renderer)
        {
            ApplyActiveRendererCapacityFloor(*entry.renderer);
        }
...
void TrackSystem::ApplyActiveRendererCapacityFloor(TrackRenderer& renderer)
{
    ...
    if (rendererVertexCapacityFloor_ == 0u && rendererFaceCapacityFloor_ == 0u) return;
...
void TrackSystem::TryPrefetchUpcomingSegment()
{
        ...
        !slidePrefetchFamilyIds_.empty())
    {
        ...
        // In stabilized mode, light prefetch already cached the next segment's
        // family ids and center. Re-entering the light-prefetch builder here
        // only dirties the working set again and burns frame time without
        // reducing the critical-path work at the slide boundary.
        if (kEnableTrackRuntimeStabilization && !prefetchRendererResident)
        {
...
bool TrackSystem::ShouldCompactTrackTextureHeapInStabilization() const
{
    ...
    constexpr uint32_t kHeapPctCompactThreshold = 58u;
    constexpr uint16_t kSlackSlotCompactThreshold = 48u;
    constexpr uint16_t kTrackTexCompactThreshold = 144u;
    if (heapPct >= kHeapPctCompactThreshold && slackSlots >= kSlackSlotCompactThreshold)
    {
        return true;
    }
    return trackTexUsed >= kTrackTexCompactThreshold && slackSlots >= kSlackSlotCompactThreshold;
}
...
uint32_t TrackSystem::EstimateLowWorkRamRetainedBytes() const
{
    ...
    bytes += VectorCapacityBytesSafe(familyMergeNextScratch_);
...
TrackSystem::LowWorkCategoryBreakdown TrackSystem::CaptureLowWorkBreakdown() const
{
    ...
    out.familyCache += VectorCapacityBytesSafe(familyMergeNextScratch_);
    ...
    out.transient += VectorCapacityBytesSafe(slideRollbackFamilyIdsScratch_);
    out.transient += VectorCapacityBytesSafe(slideRollbackFaceRankOffsetsScratch_);
...
bool TrackSystem::TrimWorkRamRetainedCapacities(bool aggressive, int32_t* outFreeDelta)
{
    ...
    const size_t segmentCatalogTarget = (totalSegmentCount_ > 0)
        ? (static_cast<size_t>(totalSegmentCount_) + 1u)
        : 0u;
    const size_t faceCapacityFloor = static_cast<size_t>(slotFaceCapacityFloor_);

    trimmed |= TrimVectorSlack(g_textCache, g_textCache.size(), aggressive);
    ...
    trimmed |= TrimVectorSlack(g_trackPendingRetiredTextureSlots,
                               ...
                               aggressive);
    trimmed |= TrimVectorSlack(activeWindowEntryIndexBySegmentId_, segmentCatalogTarget, aggressive);
    trimmed |= TrimVectorSlack(activeWindowLogicalRankBySegmentId_, segmentCatalogTarget, aggressive);
    ...
    trimmed |= TrimVectorSlack(seg1FamilySlots_,
                               aggressive ? seg1FamilySlots_.size() : (seg1FamilySlots_.size() + 16u),
                               aggressive);
    trimmed |= TrimVectorSlack(familyMergeCurrentWindowScratch_,
                               ...
                               aggressive);
    trimmed |= TrimVectorSlack(familyMergeNextScratch_,
                               aggressive ? familyMergeNextScratch_.size()
                                          : (familyMergeNextScratch_.size() + 16u),
                               aggressive);
    ...
    if (!kEnableDeterministicStabilizedSlide)
    {
        trimmed |= TrimVectorSlack(slideBackBuffer_.incomingFaceSlots, aggressive ? 0u : 128u, aggressive);
        for (auto& update : slideBackBuffer_.boundaryUpdates)
        {
            const size_t keep = aggressive ? 0u : (update.preparedFaceSlots.size() + 8u);
            trimmed |= TrimVectorSlack(update.preparedFaceSlots, keep, aggressive);
        }
    }
    ...
    for (size_t i = 0; i < segmentRenderers_.size(); ++i)
    {
        ...
        trimmed |= TrimVectorSlack(lod.workingSetFamilies,
                                   aggressive ? lod.workingSetFamilies.size() : (lod.workingSetFamilies.size() + 8u),
                                   aggressive);
        trimmed |= TrimVectorSlack(lod.workingSetLodIndices,
                                   aggressive ? lod.workingSetLodIndices.size() : (lod.workingSetLodIndices.size() + 8u),
                                   aggressive);
        trimmed |= TrimVectorSlack(lod.workingSetSlots,
                                   aggressive ? lod.workingSetSlots.size() : (lod.workingSetSlots.size() + 8u),
                                   aggressive);
...
void TrackSystem::RunWorkRamMaintenance(bool windowSlid)
{
    ...
    const uint32_t trackOwnedHwrBytes =
        ResolveTrackOwnedHighWorkBytesForPressure(EstimateWorkRamRetainedBytes());
    const uint32_t trackOwnedLwrBytes = EstimateLowWorkRamRetainedBytes();
    const bool trackOwnsLittleHwr =
        trackOwnedHwrBytes <= static_cast<uint32_t>(kWorkRamTrackOwnedBypassBytes);
    const bool trackOwnsLittleLwr =
        trackOwnedLwrBytes <= static_cast<uint32_t>(8u * 1024u);
    ...
    if (kEnableTrackRuntimeStabilization)
    {
        ...
        if (pressure != MemoryPressureLevel::Normal)
        {
            ...
            // 8x8 without materially recovering HWR. Texture-heap cleanup is handled by
            // the dedicated VDP1 compaction path.
        }
        const bool shouldTrimForWindowReuse =
            windowSlid && workRamTrimCooldown_ == 0 &&
            !criticalLowMemory && !criticalLowLowWork;
            ...
            !segmentRenderers_.empty())
        {
            ...
            if (freeValidAfterTrim && freeAfterTrim <= kWorkRamCatastrophicFloorBytes)
            {
                const size_t projectedFreeAfterRelease =
                    freeAfterTrim + ResolveTrackOwnedHighWorkBytesForPressure(EstimateWorkRamRetainedBytes());
...
bool TrackSystem::SlideActiveSegmentWindow(size_t stepCount, int8_t direction)
{
    ...
    auto finalizeSlideWindow = [&](bool allowImmediatePrefetch) -> bool
    {
        ...
        constexpr bool kEnableTrackWindowOverlayTelemetry = false;
    ...
    if (kEnableTrackRuntimeStabilization)
    {
        for (size_t step = 0; step < stepCount; ++step)
        {
            ...
            const auto hasReadyPrefetchForNextId = [&]() -> bool
            {
                return hasResidentPrefetchForNextId() &&
                       slidePrefetchLod8Ready_ &&
                       slidePrefetchFaceSlots_.size() == slidePrefetchFamilyIds_.size() &&
                       !HasMissingRequiredFaceTextureSlots(slidePrefetchFaceSlots_, &slidePrefetchFamilyIds_);
            };
        ...
        if (kEnableTrackRuntimeStabilization)
        {
            const int32_t upcomingAfterSlideId = (windowDirection_ > 0)
                ? WrapSegmentIdToRange(activeWindowStartId_ + static_cast<int32_t>(windowCount),
                                       totalSegmentCount_)
                : WrapSegmentIdToRange(activeWindowStartId_ - static_cast<int32_t>(windowCount),
                                       totalSegmentCount_);
    ...
    for (size_t ti = 0; ti < kForwardBoundaryTargets.size(); ++ti)
    {
        ...
        for (size_t fi = 0; fi < entry->lodState.faceFamilyIds.size(); ++fi)
        {
            ...
            if (TryGetFamilyLodSlot(seg1FamilySlots_, fam, targetLodIndex, existingSlot) &&
                IsVdp1TextureSlotLive(existingSlot))
            {
                continue;
            }
...
void TrackSystem::MergeCurrentWindowFamilies()
{
    ...
    auto adoptLiveWindowSlots = [&](FamilySlotVector& targetFamilies)
    {
        for (auto& entry : segmentRenderers_)
        {
            ...
            auto adoptSlot = [&](uint16_t familyId, int32_t slotHint, uint8_t fallbackLodIndex)
            {
                ...
                if (!IsVdp1TextureSlotLive(liveSlot)) return;
                ...
                {
                    resolvedLodIndex = (fallbackLodIndex <= 3u) ? fallbackLodIndex : 0u;
                }
                ...
                for (size_t fi = 0; fi < entry.lodState.workingSetFamilies.size(); ++fi)
                {
                    ...
                    const uint8_t lodIndex =
                        (fi < entry.lodState.workingSetLodIndices.size())
                            ? entry.lodState.workingSetLodIndices[fi]
    ...
    FamilySlotVector& nextFamilies = familyMergeNextScratch_;
    nextFamilies.clear();
    if (nextFamilies.capacity() < currentWindowFamilies.size())
    {
        nextFamilies.reserve(currentWindowFamilies.size());
    }
    nextFamilies.insert(nextFamilies.end(), currentWindowFamilies.begin(), currentWindowFamilies.end());
    ...
    for (const auto& oldFamily : seg1FamilySlots_)
    {
        ...
        if (FindFamilySlot(nextFamilies, oldFamily.familyId)) continue;
...
void TrackSystem::RefreshFamilyWorkingSet(bool releaseUnused)
{
    ...
    for (size_t i = 0; i < segmentRenderers_.size(); ++i)
    {
        ...
        for (size_t fi = 0; fi < entry.lodState.workingSetFamilies.size(); ++fi)
        {
            ...
            const uint8_t lodIndex =
                (fi < entry.lodState.workingSetLodIndices.size()) ? entry.lodState.workingSetLodIndices[fi] : 0u;
        ...
        totalSegmentCount_ > 0)
    {
        ...
        {
            const size_t logicalRank = kForwardBoundaryTargets[ti].logicalRank;
            const uint8_t lodIndex = kForwardBoundaryTargets[ti].lodIndex;
            if (logicalRank >= windowCount) continue;
            ...
            if (!entry || !entry->renderer || !entry->lodState.ready) continue;
            auto& mutableEntry = const_cast<SegmentRenderEntry&>(*entry);
            if (mutableEntry.lodState.workingSetCacheDirty)
            {
                (void)RebuildEntryWorkingSetCache(mutableEntry);
            }
            const auto& familySource = !mutableEntry.lodState.workingSetFamilies.empty()
                ? mutableEntry.lodState.workingSetFamilies
                : mutableEntry.lodState.faceFamilyIds;
            if (familySource.empty()) continue;
            ...
            for (size_t si = 0; si < seenCount; ++si)
            {
                Seg1FamilySlotEntry* family = FindFamilySlot(seg1FamilySlots_, seenFamilies[si]);
                if (!family) continue;
                if (family->workingRefs[lodIndex] < std::numeric_limits<uint16_t>::max())
                {
                    ++family->workingRefs[lodIndex];
                }
            }
        }
    }
...
void TrackSystem::ReleaseUnusedFamilyResourcesEndFrame()
{
    ...
    for (size_t i = 0; i < seg1FamilySlots_.size(); ++i)
    {
        ...
        for (uint8_t li = 0; li < 4; ++li)
        {
            ...
            // Stabilized mode still needs strict recycling. LOD0/8x8 used to be
...
void TrackSystem::ValidateStabilizedWindowInvariants()
{
    ...
    constexpr size_t kExpectedWindowSegments =
        static_cast<size_t>(kLodBand64Count + kLodBand32Count + kLodBand16Count + kLodBand8Count);
    ...
    FamilySlotVector expectedFamilies{};
    ...
    const bool prefetchMetadataMismatch =
        ...
        (prefetchHasLiveState && !prefetchHasMetadata);
    ...
    const bool badBands =
        ...
        bandCounts[1] != kLodBand16Count ||
        bandCounts[0] != kLodBand8Count;
    ...
    SRL::Debug::Print(1, 19, "TRK band 8:%u 16:%u 32:%u 64:%u",
                      static_cast<unsigned>(bandCounts[0]),
                      static_cast<unsigned>(bandCounts[1]),
...
{
    constexpr bool kEnableLegacyFamilyOverlayTelemetry = false;
    ...
    if constexpr (kEnableLegacyFamilyOverlayTelemetry)
    {
        ...
        SRL::Debug::Print(1, 16, "TRK band 8:%u 16:%u 32:%u 64:%u",
                          static_cast<unsigned>(segmentBands[0]),
                          static_cast<unsigned>(segmentBands[1]),
...
void TrackSystem::SetObservedCarSegmentId(int32_t segmentId)
{
    ...
    {
        if (RebuildActiveSegmentWindow(observedId, windowCount, +1))
        {
            ...
            activeWindowSwitchCooldown_ = 0;
            TryPrefetchUpcomingSegment();
            SRL::Debug::Print(1, 13, "TRK align s:%d o:%d",
                              currentStartId,
                              observedId);
            return;
        }
    ...
    bool& segment01Prepared)
{
    ...
    if (kEnableTrackRuntimeStabilization)
    {
        std::array<SegmentRenderEntry*, kTrackSegmentLimit> orderedEntries{};
        ...
        {
            if (!entry.renderer) continue;
            if (orderedCount >= orderedEntries.size()) break;
            orderedEntries[orderedCount++] = &entry;
        }
        ...
        std::sort(orderedEntries.begin(), orderedEntries.begin() + static_cast<std::ptrdiff_t>(orderedCount),
                              ...
                              const Vector3D& carWorldPosition)
{
    ...
    {
        if (pendingLodRetryCooldowns_[i] > 0) --pendingLodRetryCooldowns_[i];
    }
    ...
    if (kEnableTrackRuntimeStabilization &&
        !windowSlid &&
        runtimeSlidesThisFrame_ == 0u &&
        ShouldCompactTrackTextureHeapInStabilization())
    {
        const uint16_t texBefore = SRL::VDP1::GetTextureCount();
        ...
        if (RebuildTrackTextureResidencyForWindow())
        {
            textureHeapCompactCooldown_ = 18u;
            ...
            if constexpr (kEnableTrackOverlayRows16To22)
            {
                SRL::Debug::Print(1, 17, "TRK tex compact tb:%u ta:%u hp:%u>%u",
                                  static_cast<unsigned>(texBefore),
                                  static_cast<unsigned>(SRL::VDP1::GetTextureCount()),
    ...
    bool shouldRunPostSlideMaintenance = false;
    if (kEnableTrackRuntimeStabilization && slidThisFrame)
    {
        ...
        const bool postSlideHighPressure =
            postSlideFreeValid &&
            postSlideFreeBytes <= (kWorkRamHardFloorBytes + (16u * 1024u));
        const bool postSlideLowPressure =
            postSlideLowFreeValid &&
            postSlideLowFreeBytes <= kLowWorkRamSoftFloorBytes;
        shouldRunPostSlideMaintenance =
            postSlideHighPressure ||
            postSlideLowPressure ||
        ...
        totalSegmentCount_ > 0)
    {
        ...
        for (size_t logicalRank = 0; logicalRank < windowCount; ++logicalRank)
        {
            ...
            const bool needsUpdate = entry->lodState.hasPerFaceRankOffsets
                ? (entry->lodState.currentLodIndex != desiredLodIndex ||
                   entry->lodState.currentBaseRank != static_cast<int16_t>(logicalRank) ||
                   HasMissingRequiredFaceTextureSlots(entry->lodState.currentFaceSlots,
                                                      &entry->lodState.faceFamilyIds))
                : (entry->lodState.currentLodIndex != desiredLodIndex ||
                   HasMissingRequiredFaceTextureSlots(entry->lodState.currentFaceSlots,
...
{
    constexpr bool kEnableLegacyTrackOverlayTelemetry = false;
    ...
    if constexpr (kEnableLegacyTrackOverlayTelemetry)
    {
                          ...
                          static_cast<unsigned>(releasedPrefetchNowThisFrame_));
        {
            ...
            SRL::Debug::Print(1, 24, "SH2 ms:%u md:%u mf:%u ss:%u sx:%u lf:%u",
                              ...
                              static_cast<unsigned>(prod.slaveMaxJobTicks),
