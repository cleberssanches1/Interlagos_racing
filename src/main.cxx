...
{
    CarAnchorBootstrapFallback fallback{};
    if (!decision.shouldUseAnchorFallback)
    {
        return fallback;
    }

    fallback.hasVisualYawOffset =
        ComputeCarVisualYawOffsetFromAnchors(decision.anchors, fallback.visualYawOffsetDeg);
    if (!decision.hasValidAnchors)
    {
        return fallback;
    }

    const int32_t dxRaw = (decision.anchors.front.X - decision.anchors.rear.X).RawValue();
    const int32_t dzRaw = (decision.anchors.front.Z - decision.anchors.rear.Z).RawValue();
                                                    ...
                                                    Game::IAudioEvents* audioEvents)
{
    ...
    loopContext.SetEnableBg(enableBg);
    loopContext.SetRenderTrack(renderTrack);
    loopContext.SetRenderCar(renderCar);
    loopContext.SetRenderAxes(renderAxes);
    loopContext.SetTrackSystemReady(trackSystemReady);
    loopContext.SetVerboseFrameLogs(kVerboseFrameLogs);
    loopContext.SetLogTrack(logTrack);
    loopContext.SetLogCar(logCar && kCarLogs);
    loopContext.SetEnableRuntimeStatsLogs(enableRuntimeStatsLogs);
    loopContext.SetEnableMinimalFpsOverlay(enableMinimalFpsOverlay);
    // Simulation and lockstep policy are independent toggles.
    loopContext.SetEnableSlaveForCarPrepare(enableRuntimeSimulation && enableSlaveForCarPrepare);
    loopContext.SetEnableSlaveForSimulation(enableRuntimeSimulation && enableSlaveSimulation);
    loopContext.SetSlaveSimulationLockstep(slaveSimulationLockstep);
    loopContext.SetEnableManualGouraudCopy(enableManualGouraudCopy);
    loopContext.SetAutoLapEnabledOnStart(autoLapEnabledOnStart);
    loopContext.SetAllowAutoLapInputToggle(allowAutoLapInputToggle);
    ...
    loopContext.SetRenderCarShadowModel(renderCarShadowModel);
    loopContext.SetSbaLoaded(sbaLoaded);
